package com.example.oria.ui.theme

import androidx.compose.ui.unit.dp

/**
 * File to store all global const of the project
 *
 */


val TEXT_FIELD_PADDING_VERTICAL = 10.dp
val TEXT_FIELD_PADDING_HORIZONTAL = 22.dp
val TEXT_FIELD_ROUNDING_CORNER = 20.dp
val TEXT_FIELD_WIDTH = 320.dp
val TEXT_FIELD_HEIGHT = 73.dp

const val GRID_ROW = 20
const val GRID_COLUMN = 9
const val TIMEOUT_MILLIS = 5_00L


// Return Values
const val NO_ERROR = 0
const val NO_RESPONSE = 1

const val ERROR_LOGIN = 101
const val ERROR_LOGIN_EMAIL = 102
const val ERROR_LOGIN_ID = 103
const val ERROR_PASSWORD = 104

const val ERROR_INCORRECT_USERNAME_FORMAT = 201
const val ERROR_INCORRECT_PASSWORD_FORMAT = 202
const val ERROR_INCORRECT_NAME_FORMAT = 203
const val ERROR_INCORRECT_EMAIL_FORMAT = 204

const val ERROR_REQUEST = 400
const val ERROR_SERVER = 500


// SERVER values
const val REQUEST_TIMEOUT = 6000L
const val CONNECTION_TIMEOUT = 6000L
const val URL_BASE = "http://10.0.2.2:8000"

